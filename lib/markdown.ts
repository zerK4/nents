...
export function getPreviousNext(path: string) {
  let index = page_routes.findIndex(({ href }) => href == `/${path}`);

  let minusOne = page_routes[index - 1];
  while (minusOne && !minusOne.title.trim()) {
    minusOne = page_routes[--index - 1];
  }

  let plusOne = page_routes[index + 1];
  while (plusOne && !plusOne.title.trim()) {
    plusOne = page_routes[++index + 1];
  }
